...
var (
	...
	thingName   = flag.String("name", "dynamic-pop-thing", "Thing name")
	secretStore = flag.String("secrets", "", "Path to pre-created secret store")
	debug       = flag.Bool("debug", false, "Enable debug output")
)

// popRegistration registers and authenticates a Thing with AM, using the Proof of Possession registration and
// authentication methods.
func popRegistration() (err error) {
	u, err := url.Parse(*urlString)
	if err != nil {
		return err
	}
	store := secrets.Store{Path: *secretStore}
	thingKey, err := store.Signer(*thingName)
	if err != nil {
		return err
	}
	...
	deviceBuilder := builder.Thing().
		...
		InRealm(*realm).
		WithTree(*authTree).
		HandleCallbacksWith(callback.ProofOfPossessionHandler(*thingName, *audience, thingKid, thingKey))
...
func main() {
	...
	if err := popRegistration(); err != nil {
